	...
	return (
		<section id="home" className="relative flex min-h-screen items-center overflow-hidden bg-background">
			<div className="absolute inset-0">
				<Carousel className="h-full w-full pointer-events-none" opts={{ loop: true }} setApi={setCarouselApi}>
					<CarouselContent className="h-full">
						{slides.map((slide) => (
							<CarouselItem key={slide.id} className="h-full">
								<div className="relative h-full w-full">
									<img src={slide.image} alt={slide.id} className="h-full w-full object-cover" />
								</div>
							</CarouselItem>
						))}
					</CarouselContent>
						...
						{slides.map((_, index) => (
							<button
								...
								className={cn(
									...
									index === currentSlide ? "w-6 bg-primary" : "w-2 bg-muted-foreground/50",
								)}
